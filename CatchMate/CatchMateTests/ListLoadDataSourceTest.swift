...
final class ListLoadDataSourceTest: XCTestCase {
    ...
    func testDefaultsFilter() {
        ...
        dataSource.loadPostList(pageNum: 1, gudan: [], gameDate: "", people: 0)
            .subscribe (onNext: { result in
                print(result)
                XCTAssertTrue(!result.isEmpty, "API 호출 결과로 빈 목록이 아닌 값이 반환됩니다.")
                expectation.fulfill()
            }, onError: { error in
    ...
    func testNumberFilter() {
        let expectation = self.expectation(description: "Post Load API Request - NumberFilter")
        dataSource.loadPostList(pageNum: 1, gudan: [], gameDate: "", people: 4)
            .subscribe (onNext: { result in
                print(result)
                XCTAssertTrue(!result.isEmpty, "API 호출 결과로 빈 목록이 아닌 값이 반환됩니다.")
                expectation.fulfill()
    ...
    func testGudanFilter() {
        let expectation = self.expectation(description: "Post Load API Request - DefaultsFilter")
        dataSource.loadPostList(pageNum: 1, gudan: ["다이노스"], gameDate: "", people: 0)
            .subscribe (onNext: { result in
                print(result)
                XCTAssertTrue(result.isEmpty, "API 호출 결과로 빈 목록이 반환됩니다.")
                expectation.fulfill()
            }, onError: { error in
                if let afError = error as? AFError {
                    switch afError {
