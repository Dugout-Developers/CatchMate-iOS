...
final class CheerTeamPickerViewController: BasePickerViewController, View {
    ...
    func bind(reactor: AddReactor) {
        ...
        teams.bind(to: tableView.rx.items(cellIdentifier: "CheerTeamTableViewCell", cellType: CheerTeamTableViewCell.self)) { row, team, cell in
            if let myTeamStr = SetupInfoService.shared.getUserInfo(type: .team), let myTeam = Team(rawValue: myTeamStr)  {
                cell.setupData(team: team, isClicked: myTeam == team)
            }
            cell.selectionStyle = .none
            cell.checkButton.rx.tap
                .map { Reactor.Action.changeCheerTeam(team) }
