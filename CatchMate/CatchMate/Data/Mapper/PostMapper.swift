...
final class PostMapper {
    ...
    func dtoToDomain(_ dto: PostDTO) -> Post? {
        ...
        return Post(id: String(dto.boardId), title: dto.title, writer: SimpleUser(userId: writer.userId, nickName: writer.nickName, picture: writer.profileImageUrl, favGudan: writerTeam, gender: writerGender, birthDate: writer.birthDate, cheerStyle: writerCheerStyle), homeTeam: homeTeam, awayTeam: awayTeam, cheerTeam: cheerTeam, date: gameDate.date, playTime: gameDate.playTime, location: gameInfo.location, maxPerson: dto.maxPerson, currentPerson: dto.currentPerson, preferGender: gender, preferAge: preferAges, addInfo: dto.content, chatRoomId: dto.chatRoomId, gameDateString: dto.gameInfo.gameStartDate ?? "")
    }
